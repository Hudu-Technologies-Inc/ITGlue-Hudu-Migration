...
By default, configurations are imported into one layout unless `SplitConfigurations` is enabled, which creates one Hudu layout per exact IT Glue configuration type name. To group similar types into a smaller set of layouts, set `SmartSplitConfigurations` to `$true` or set `ConfigurationSplitMode` to `"Smart"` in your settings. Smart split uses normalized tokens from `configuration-type-name` and `configuration-type-kind`, fuzzy token similarity, and built-in families such as workstations, servers, printers, and network gear. Type-name tokens are weighted first; type-kind tokens are mainly used as a fallback when the type name is generic, such as `Network Device` with kind `router`. Internet-facing records such as WAN connections, internet domains, IP addresses, websites, and network security appliances are grouped under `Internet / WAN`. Blank or placeholder type names are grouped as `Configurations`. `SmartSplitConfigurationMaxCategories` defaults to `20`. If smart split has to collapse low-volume leftovers to stay under the cap, the overflow layout is named from representative leftovers, for example `Other - Camera/Imaging/Document Scanner/Xray`.

The selected smart groups are printed during configuration layout collision detection and written to `SmartConfigurationSplits-Preflight.json` and `SmartConfigurationSplits-Preflight.csv` in the migration logs. The final import grouping is also written to `SmartConfigurationSplits.json` before configuration import starts.
