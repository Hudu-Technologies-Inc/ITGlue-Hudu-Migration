...
if ($ResumeFound -eq $true -and (Test-Path "$MigrationLogs\AssetLayouts.json")) {
    ...
    $MatchedLayouts = foreach ($ITGLayout in $FlexLayouts) {
        if ($skipIntegratorLayouts -and $true -eq $skipIntegratorLayouts){
            if ("$($ITGLayout.attributes.name)" -ilike "*(auto)*"  -or "$($ITGLayout.attributes.name)" -ilike "*9fd7*" -or "$($ITGLayout.attributes.name)" -ilike "*b857*" -or  "$($ITGLayout.attributes.name)" -ilike "*(liongard)*" -or "$($ITGLayout.attributes.name)" -ilike "*datto*rmm*" -or "$($ITGLayout.attributes.name)" -ilike "*connectwise*"  -or "$($ITGLayout.attributes.name)" -ilike "*autotask*" ){
    ...
    if ($ImportFlexibleAssetLayouts -eq $true) {
        ...
        foreach ($UpdateLayout in $MatchedLayouts) {
            if ($skipIntegratorLayouts -and $true -eq $skipIntegratorLayouts){
                if ("$($UpdateLayout.Name)" -ilike "*(auto)*" -or "$($UpdateLayout.Name)" -ilike "*b857*" -or "$($UpdateLayout.Name)" -ilike "*9fd7*" -or "$($UpdateLayout.Name)" -ilike "*(liongard)*" -or "$($UpdateLayout.Name)" -ilike "*datto*rmm*" -or "$($UpdateLayout.Name)" -ilike "*connectwise*"  -or "$($UpdateLayout.Name)" -ilike "*autotask*" ){ 
...
Write-Host "Asset rich text link replacement will run after attachments are uploaded so IT Glue links, image links, and attachment links can be handled in one asset update." -ForegroundColor Cyan
...
$AttachmentUrlLookupForReplacement = if ($AttachmentUrlMapForReplacement -and $AttachmentUrlMapForReplacement.Count -gt 0) {
    New-AttachmentUrlReplacementLookup -UrlMap (ConvertTo-RelativeUrlMap -UrlMap $AttachmentUrlMapForReplacement)
} else {
    @{}
}
