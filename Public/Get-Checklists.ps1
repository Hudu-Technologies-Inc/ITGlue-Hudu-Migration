...
.DESCRIPTION
    This function queries the IT Glue Checklist template Tasks endpoint to retrieve items for a specific checklist template.
    It requires a JWT token for authentication, which can be obtained from browser developer tools when logged into the IT Glue web interface.
    Supports filtering by checklist ID, sorting, and pagination.

.PARAMETER JWTAuthToken
    The JWT token required for authenticating to the Checklist Templates Tasks endpoint.

.PARAMETER filter_checklist_id
    The ID of the checklist template to retrieve items for. This parameter is mandatory.
...
.EXAMPLE
    Get-ITGlueChecklistTemplateItems -JWTAuthToken "your_jwt_token" -filter_checklist_id 3510640306421985
    Retrieves all checklist items for the specified checklist ID.

.EXAMPLE
    Get-ITGlueChecklistTemplateItems -JWTAuthToken "your_jwt_token" -filter_checklist_id 3510640306421985 -page_size 1000
    Retrieves up to 1000 checklist items for the specified checklist ID.
