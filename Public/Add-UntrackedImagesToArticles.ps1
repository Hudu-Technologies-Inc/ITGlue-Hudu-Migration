...
# any article content. This is intended to produce a review queue for possible orphaned images
function Add-UntrackedImagesToArticles {
    param (
        [array]$ForArticles = @(),
        $EmbeddableImageExtensions = @(".jpg", ".jpeg",".png",".gif",".bmp",".webp",".svg",".apng",".avif",".ico",".jfif",".pjpeg",".pjp"),
        [string]$AddendumBeginMarker = "<!-- hudu-images-addendum:begin -->",
        [string]$AddendumEndMarker = "<!-- hudu-images-addendum:end -->",
        [string]$addendumHeader="Other Associated Images",
        [string]$addendumNote="The following image attachments were associated with this article but were not found inline in article content.",
        [string]$huduAddendumClass= "hudu-image-addendum"
    )

    if ($null -eq $ForArticles -or $ForArticles.count -le 1){
        $ForArticles = get-huduarticles
    }
    ...
    function New-HuduImageAddendumHtml {
        param(
            [Parameter(Mandatory)]
            [array]$Images,
            [string]$addendumHeader,
            [string]$addendumNote,
            [string]$huduAddendumClass
        )
        ...
        $null = $html.AppendLine("<h2>$addendumHeader</h2>")
        $null = $html.AppendLine("<p>$addendumNote</p>")
        $null = $html.AppendLine("<div class=""$huduAddendumClass"">")
    ...
    return $(Set-HuduOwnerArticleImageAddendums -Apply -addendumHeader $addendumHeader -addendumNote $addendumNote -huduAddendumClass $huduAddendumClass)
}
