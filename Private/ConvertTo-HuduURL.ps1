...
function Test-HuduAssetFieldIsITGlueMetadata {
    param(
        [AllowNull()]
        $Field
    )

    $label = Get-NormalizedHuduFieldLabel -Label (Get-HuduAssetFieldLabel -Field $Field)
    return ($label -in @('itglue_url', 'itglue_id', 'imported_from_itglue'))
}
